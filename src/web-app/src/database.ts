...
import { CACHE_TIME_SECONDS, options } from "./config";
import { getFromCache, memcached } from "./memcache";
...
async function executeQuery(query: string, data: any) {
    let connection;
    try {
...
export async function getPopular(maxCount: number) {
    const query =
    ...
    return (await executeQuery(query, [maxCount])).map((row: any[]) => ({
        tweetId: row?.[0],
        sentiment: row?.[1],
        count: row?.[2],
        author: row?.[3],
...
export async function getEvents() {
    return (await executeQuery("SELECT event_type, count FROM events ORDER BY count DESC", [])).map((row: any[]) => ({
